...
class Menu(Scene):
    def __init__(self, manager):
        ...
        try:
            ...
            self.Akane = pygame.Surface((0, 0), pygame.SRCALPHA)
            self.akane_x = 0
            self.akane_y = 0
    ...
    def handle_events(self, event):
        ...
        if event.type == pygame.KEYDOWN:
            if event.key in (pygame.K_KP_ENTER, pygame.K_RETURN):
                ...
                self.choose = self.choose + 1 if self.choose < len(self.menu_items) else 1
            elif event.key in (pygame.K_w, pygame.K_UP):
                self.choose = self.choose - 1 if self.choose > 1 else len(self.menu_items)
